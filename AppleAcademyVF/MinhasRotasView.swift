...
struct CardViewMinhasRotas: View {
    var imageName: String
    var rota: RotaContentView
    var onEdit: (RotaContentView) -> Void
    var onDelete: (RotaContentView) -> Void
    var onPin: (RotaContentView) -> Void
...
struct MinhasRotasView: View {
    ...
    @State private var rotaParaEditar: RotaContentView? = nil
    @State private var isShowingDeleteAlert = false
    @State private var rotaParaExcluir: RotaContentView? = nil
        ...
        NavigationStack {
                    ...
                    ScrollView {
                        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 20) {
                            ForEach(appState.rotas) { rota in
                                NavigationLink(destination: DetalhesMinhasRotasView(rota: rota)) {
                                    CardViewMinhasRotas(
            ...
            .navigationBarItems(trailing:
                NavigationLink(destination: CriarMinhasRotasView { nomeRota in
                    let novaRota = RotaContentView(nome: nomeRota)
